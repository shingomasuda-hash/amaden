...
- 手書きの分解整備成績書PDFは **Claude（Anthropic API）** が読み取り、確認画面（読み取り結果／作業内容／測定値）に反映します（Vercel Serverless Functionで実行、APIキーはサーバー側のみ）。1回ごとの読み取りコスト（概算）を案件ごと・全体で記録し、管理者画面の「先方とのやり取り」タブで確認できます。
...
### 10. AI読み取り（Claude API）を有効にする

手書きPDFの自動読み取りには [Anthropic Console](https://console.anthropic.com/) のAPIキーが必要です。

1. Anthropic Consoleで自社（弊社）のアカウントを作成し、クレジット（プリペイド）を追加してAPIキーを発行
2. Vercelのプロジェクト → Settings → Environment Variables に `ANTHROPIC_API_KEY` を追加（**このキーは絶対に公開しない・コミットしない**）
3. 再デプロイすると、案件のアップロード画面から選んだPDFがAIで読み取られるようになります

**費用の考え方（パターンA）**：APIキー・請求は弊社のAnthropicアカウントで一括管理し、利用料は請求書などで別途先方に請求する運用です。Anthropic側での先方アカウント連携などは不要です。読み取り1回ごとの概算コストは案件に記録され、管理者画面「先方とのやり取り」タブの一覧・合計で確認できます（金額は参考値です。正確な請求額は console.anthropic.com の使用状況をご確認ください）。

APIキーを設定しない場合でも、他の機能（ログイン・チャット・管理者画面など）は問題なく動作します。読み取りを試みた際に「サーバーにANTHROPIC_API_KEYが設定されていません」というエラーが表示されるだけです。
